...
export async function batchCompare(
  ...
  return out
    .filter((el) => el.score <= (threshold || 0.9))
    .map((el) => el.address);
}

async function main() {
  const cryptokitties = await getByteCode(
    "0x06012c8cf97bead5deae237070f9587f8e7a266d"
  );
  const bridge1 = await getByteCode(
    "0xd9e1ce17f2641f24ae83637ab66a2cca9c378b9f"
  );
  const bridge2 = await getByteCode(
    "0x7a250d5630b4cf539739df2c5dacb4c659f2488d"
  );
  const cryptopuppies = await getByteCode(
  ...
  permutations.forEach(([a, b]) => {
    ...
    console.log(
      `[${jaccardIndex(getSigHashes(a.opcodes), getSigHashes(b.opcodes))}] ${
        a.name
      }<->${b.name}`
    );
  });
