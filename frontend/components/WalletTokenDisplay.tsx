  ...
  return (<div>
    <button className="glass my-6 py-2 px-4 w-1/2 flex text-left" onClick={fetchChildren}>
      <div className="my-auto mr-4">
        {props.metadata.logo ?
          <Image className={isNFT ? 'rounded-md' : 'rounded-full'} src={props.metadata.logo} width={40} height={40}/> :
          <Avatar
            size={40}
            variant="marble"
      ...
      {isNFT ?
        <div>
          ...
          <a className="opacity-30 text-sm font-normal underline underline-offset-2" href={`https://etherscan.io/address/${props.contractAddress}`}>View on Etherscan</a>
        </div> :
        <div>
